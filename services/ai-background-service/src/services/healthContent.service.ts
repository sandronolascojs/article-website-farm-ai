...
export class GenerateHealthContentService {
  ...
  constructor(
    logger: Logger,
    agent: HealthFoodArticleGeneratorAgent,
  ) {
    this.logger = logger;
    this.agent = agent;
  ...
  async generateHealthArticle(count: number = 1) {
    for (let i = 0; i < count; i++) {
      const existingTitles = await this.db.query.articles.findMany({
        columns: {
          title: true,
        },
      ...
      if (!category) {
        const [insertedCategory] = await this.db.insert(categories).values({
          name: result.category,
        }).returning();
        category = insertedCategory
        this.logger.info(`Category not found, inserting: ${result.category} with id: ${insertedCategory?.categoryId}`);
      }
